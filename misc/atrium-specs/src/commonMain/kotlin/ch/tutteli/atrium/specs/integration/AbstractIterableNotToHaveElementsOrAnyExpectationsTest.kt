...
abstract class AbstractIterableNotToHaveElementsOrAnyExpectationsTest(
    ...
    fun empty_collection__does_not_throw() = nonNullableCases(
        ...
        ch.tutteli.atrium.api.verbs.internal.expect(fluentEmpty()).notToHaveElementsOrAnyFun { toBeLessThan(1.1) }
    }
    ...
    @TestFactory
    fun nullable_cases() = testFactory(notToHaveElementsOrAnyNullableSpec) {
        describe("iterable ${oneToSevenNullable().toList()}") {
            describe("happy cases (do not throw)") {
                it("$toEqualFun(1.1)") {
                    expect(oneToSevenNullable()).notToHaveElementsOrAll { toEqual(1.1) }
                }
                it("null") {
                    expect(oneToSevenNullable()).notToHaveElementsOrAll(null)
                }
            }
        }
        describe("failing cases") {
            it("$toEqualFun(2.1)") {
                expect {
                    expect(oneToSevenNullable()).notToHaveElementsOrAll { toEqual(2.1) }
                }.toThrow<AssertionError> {
                    message {
        ...
        describe("iterable ${oneToSeven().toList()}") {
            it("null, throws an AssertionError") {
                expect {
                    expect(oneToSeven() as Iterable<Double?>).notToHaveElementsOrAll(null)
                }.toThrow<AssertionError> {
                    message {
                        toContain.exactly(1).values(
                            "$rootBulletPoint$notToHaveElementsOrAnyDescr: $separator",
